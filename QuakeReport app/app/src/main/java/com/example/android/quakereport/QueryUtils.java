...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject main_query = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray features_array = main_query.getJSONArray("features");

            for (int i = 0; i < features_array.length(); i++) {
                JSONObject earthquake = features_array.getJSONObject(i);
                JSONObject earthquake_properties = earthquake.getJSONObject("properties");
                double mag = earthquake_properties.getDouble("mag");
                String place = earthquake_properties.getString("place");
                Long time = earthquake_properties.getLong("time");
                String url = earthquake_properties.getString("url");
